#!/usr/bin/env bash
##
## Replay messages from prod into prod
##
## Usage:
...
##   creating a pod in the prod namespace and running the sqs-utils.py send script. This allows us to use the service
...
# Get queue url
queue_url=$(kubectl exec "$pod_name" --namespace="$namespace" -- aws sqs get-queue-url --queue-name "probation-integration-prod-$QUEUE_NAME" --query QueueUrl --output text)
