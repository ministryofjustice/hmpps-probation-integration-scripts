...
RAM_CONTACT AS (
    SELECT c.NSI_ID,
           CASE WHEN rct.CODE = 'CRSAPT' THEN 'Service Delivery Appointment'
               ...
               ELSE REGEXP_REPLACE(TO_CHAR(dbms_lob.substr(c.NOTES,1000,1)),'^(Comment added by .+? on .+? at .+?'||chr(10)||')+(\w+ \w+ \w+).*', '\2', 1, 0, 'n') END CONTACT_NOTES,
           ...
           c.ATTENDED as CONTACT_ATTENDED,
           c.COMPLIED as CONTACT_COMPLIED,
...
SELECT SUBSTR(nsi.EXTERNAL_REFERENCE, -36, 36) REFERRAL_ID,
       ...
       REGEXP_REPLACE(dbms_lob.substr(nsi.NOTES,1000,1),'.+? Referral (\w+) .*', '\1', 1, 0, 'n') REFERENCE_NUMBER,
